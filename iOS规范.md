...
UIButton *setBut;
```
类名和常量应该始终使用三个字母的前缀, 例如: 框架层面的 `FMT`, 业务层面的 `YTSS`(车商精灵), `YTCM`(车达人), `YMC`(要买车)。但 Core Data 实体名称可以省略。为了代码清晰，常量应该使用相关类的名字作为前缀并使用驼峰命名法。

**推荐：**

```objc
static const NSTimeInterval FMTViewControllerNavigationFadeAnimationDuration = 0.3;
```
...
### 分类是类的功能片段， 所以在命名上需要清楚地表达其功能

**推荐:**

```objc
@interface UIViewController (FMTMediaPlaying)
...
```objc
@interface FMTAdvertisement (private)
@interface NSString (FMTAdditions)
```

分类中方法和属性必须以一个应用相关或者组织相关的前缀命名。这避免了覆盖了一个已有方法的潜在危险， 也可以减少来自不同库添加分类方法重名的问题。 （Objective-C 运行时不会申明重名的方法谁会被先调用， 这样会造成不可知的影响）

**推荐:**

```objc
@interface NSArray (FMTAccessors)
- (id)fmt_objectOrNilAtIndex:(NSUInteger)index;
@end
```
...
```objc
static NSString * const FMTAboutViewControllerCompanyName = @"Shanghai Fortune Map Trading Limited";

static const CGFloat FMTImageThumbnailHeight = 50.0;
```

**反对：**

```objc
#define CompanyName @"Shanghai Fortune Map Trading Limited"
...
```objc
typedef NS_ENUM(NSInteger, FMTAdRequestState) {
    FMTAdRequestStateInactive,
    FMTAdRequestStateLoading
};
```
...
```objc
typedef NS_OPTIONS(NSUInteger, FMTAdCategory) {
    FMTAdCategoryAutos      = 1 << 0,
    FMTAdCategoryJobs       = 1 << 1,
    FMTAdCategoryRealState  = 1 << 2,
    FMTAdCategoryTechnology = 1 << 3
};
```
...
```objc
@interface FMTAdvertisement ()

@property (nonatomic, strong) UIImage *adPrivateImage;
@property (nonatomic, strong) NSString *adPrivateString;
